 ...
 * shared/update_state.json (docs/spec/06-betrieb.md section 1).
 *
 * A file and not the database on purpose: the chain has to survive the
 * moment when current/ is being renamed, and it has to be readable by a
 * release that has not run its migrations yet.
 */
final readonly class UpdateState
{
    ...
    public static function fromArray(array $data): self
    {
        return new self(
            ...
            abgeschlossenerSchritt: isset($data['abgeschlossener_schritt'])
                ? (string) $data['abgeschlossener_schritt']
                : null,
