 ...
 *
 * On this shared hosting setup the docroot contains only the shim index.php;
 * Apache cannot serve files above it, so every asset request is rewritten to
 * the front controller and answered here. The realpath guard below is
 * security critical: shared/config.php (server key, DB credentials) lives one
 * level above the release root, and shared/var/blobs/ next to it.
 */
