...
final class ReleaseSwitcherTest extends TestCase
{
    ...
    {
        $this->base = sys_get_temp_dir() . '/vb_switch_' . uniqid('', true);
    ...
    public function testSwitchIsIdempotent(): void
    {
        ...
        $switcher->switchTo('1.1.0');
        $switcher->switchTo('1.1.0'); // e.g. a retry after a lost response
    ...
    public function testMissingUnpackedReleaseFails(): void
    {
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage('Entpacktes Release fehlt');

        new ReleaseSwitcher($this->base)->switchTo('9.9.9');
    }
    ...
    // ---- docroot self-healing (docs/spec/06-betrieb.md section 1) ----
     ...
     */
    private function prepareDocrootTemplates(string $releaseDir, array $dateien): void
    {
        mkdir($this->base . '/' . $releaseDir . '/docker/web', 0775, true);
        foreach ($dateien as $name => $inhalt) {
            file_put_contents($this->base . '/' . $releaseDir . '/docker/web/' . $name, $inhalt);
        }
    }

    public function testDocrootFilesAreUpdatedWhenUntouched(): void
    {
        mkdir($this->base . '/web', 0775, true);
        file_put_contents($this->base . '/web/.htaccess', "alte CSP\n");
        $this->prepareDocrootTemplates('releases/_prev', ['.htaccess' => "alte CSP\n"]);
        $this->prepareDocrootTemplates('current', ['.htaccess' => "neue CSP\n", '.user.ini' => "neu\n"]);

        $meldungen = new ReleaseSwitcher($this->base)->refreshDocrootFiles();

        self::assertSame("neue CSP\n", file_get_contents($this->base . '/web/.htaccess'));
        self::assertSame("neu\n", file_get_contents($this->base . '/web/.user.ini'), 'a missing file is created');
        self::assertCount(2, $meldungen);
    }

    /**
     * A hand-tuned .htaccess - an IP block, a basic-auth stanza - must not
     * be silently reverted by an update. It is reported instead.
     */
    public function testAHandEditedDocrootFileIsKept(): void
    {
        mkdir($this->base . '/web', 0775, true);
        file_put_contents($this->base . '/web/.htaccess', "eigene Regeln\n");
        $this->prepareDocrootTemplates('releases/_prev', ['.htaccess' => "alte CSP\n"]);
        $this->prepareDocrootTemplates('current', ['.htaccess' => "neue CSP\n"]);

        $meldungen = new ReleaseSwitcher($this->base)->refreshDocrootFiles();

        self::assertSame("eigene Regeln\n", file_get_contents($this->base . '/web/.htaccess'));
        self::assertStringContainsString('von Hand geändert', implode(' ', $meldungen));
    }

    public function testDocrootRefreshIsSilentWhenEverythingIsCurrent(): void
    {
        mkdir($this->base . '/web', 0775, true);
        file_put_contents($this->base . '/web/.htaccess', "CSP\n");
        $this->prepareDocrootTemplates('current', ['.htaccess' => "CSP\n"]);

        self::assertSame([], new ReleaseSwitcher($this->base)->refreshDocrootFiles());
    }

    public function testDocrootRefreshLeavesAForeignLayoutAlone(): void
    {
        $this->prepareDocrootTemplates('current', ['.htaccess' => "CSP\n"]);

        self::assertSame([], new ReleaseSwitcher($this->base)->refreshDocrootFiles());
        self::assertDirectoryDoesNotExist($this->base . '/web');
    }
    ...
    public function testMaintenanceFlagCarriesItsReason(): void
    {
        ...
        $switcher->setMaintenanceFlag('Update');
        $inhalt = (string) file_get_contents($this->base . '/shared/maintenance.flag');

        self::assertStringContainsString('Update', $inhalt, 'the admin banner shows why the site is down');
