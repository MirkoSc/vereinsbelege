...
/**
 * Tests for tools/hosting-check.php (docs/spec/06-betrieb.md section 5).
 *
 * The hosting check itself stays dependency-free - it is uploaded by FTP into
 * a bare directory on a webspace that has neither Composer nor vendor/. Its
 * TESTS do not have to be, and since milestone M1-1 they are not: the checks
 * below are the ones the standalone runner carried, unchanged, driven by
 * PHPUnit so that `vendor/bin/phpunit` is the single test command.
 *
 * Each case is one entry in the provider, so a failure names the probe rather
 * than a line number. HcAssert exists because the cases keep the assertion
 * vocabulary of the standalone runner (same/true/contains/notContains); it
 * forwards to PHPUnit, which is what makes the failures readable.
 */
final class HostingCheckTest extends TestCase
{
    /**
     * The script is a flat file of hc_* functions, so it is required once and
     * then simply called. HOSTING_CHECK_LOAD_ONLY keeps it from running its
     * own main() on include.
     */
    private static function loadHostingCheck(): void
    {
        if (!defined('HOSTING_CHECK_LOAD_ONLY')) {
            define('HOSTING_CHECK_LOAD_ONLY', true);
        }
        require_once dirname(__DIR__) . '/tools/hosting-check.php';
    }

    #[DataProvider('faelle')]
    public function testHostingCheck(\Closure $fall): void
    {
        $fall(new HcAssert());
    }
    ...
    {
        self::loadHostingCheck();

        $faelle = [];

        $faelle['hc_parse_bytes versteht ini-Kurzschreibweisen'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_parse_bytes meldet unbegrenzt und Unlesbares'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_format_bytes formatiert lesbar'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_token_matches ist streng'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_status_rank ordnet nach Schwere'] = static function (HcAssert $t): void {
            $t->true(
        ...
        $faelle['hc_rate_min bewertet Mindestwerte'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_row liefert die feste Zeilenform'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_summarize zählt und findet das schlechteste Ergebnis'] = static function (HcAssert $t): void {
            $groups = [
                ['id' => 'a', 'label' => 'A', 'rows' => [
        ...
        $faelle['hc_redact entfernt Geheimnisse aus der Ausgabe'] = static function (HcAssert $t): void {
            $redacted = hc_redact([
                'smtp_pass' => 'geheim',
                'db_pass' => 'geheim',
                'token' => 's3cret',
        ...
        $faelle['hc_pad füllt auf und kürzt mit Auslassung'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_status_symbol markiert jeden Status'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_params_from_argv liest Optionen'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_params_from_request lässt POST gewinnen'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_params_from_request ignoriert Nicht-Strings'] = static function (HcAssert $t): void {
            $params = hc_params_from_request(['a' => ['array'], 'b' => 'ok'], [2 => 'numerischer Schlüssel']);
            $t->same(['b' => 'ok'], $params);
        };

        $faelle['hc_presented_token nimmt Parameter oder Kopfzeile'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_format kennt Standard und Rückfall'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_mode akzeptiert nur bekannte Tests'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_build_report liefert alle Gruppen'] = static function (HcAssert $t) use ($report): void {
        ...
        $faelle['jede Zeile hat die feste Form und einen erlaubten Status'] = static function (HcAssert $t) use ($report): void {
        ...
        $faelle['der Bericht deckt die Prüfpunkte der Spec ab'] = static function (HcAssert $t) use ($report): void {
            $ids = [];
            foreach ($report['groups'] as $group) {
        ...
        $faelle['Datenbank und ausgehende Verbindungen werden ohne Angaben übersprungen'] = static function (HcAssert $t) use ($report): void {
            $byId = [];
            foreach ($report['groups'] as $group) {
                foreach ($group['rows'] as $row) {
                    $byId[$row['id']] = $row;
        ...
        $faelle['hc_public_url_for leitet die öffentliche Adresse aus dem Request ab'] = static function (HcAssert $t): void {
            $t->same(
        ...
        $faelle['hc_public_url_for verweigert unbrauchbare Request-Daten'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_htaccess_deny sperrt für alte und neue Apache-Versionen'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_group_htaccess prüft nichts ohne echten Webserver'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_outbound_targets prüft die Anbieter der Spec'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_outbound_targets nimmt einen eigenen KI-Server auf'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_http_probe scheitert ohne Ausnahme bei unbrauchbarer URL'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_render_json liefert gültiges, lesbares JSON'] = static function (HcAssert $t) use ($report): void {
        ...
        $faelle['hc_render_text zeigt Status, Befund und Annahme'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_render_html maskiert Sonderzeichen und bringt kein Skript mit'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_group_filesystem räumt seine Prüfdateien restlos auf'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_group_filesystem unterscheidet DocumentRoot, darin und darüber'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_group_filesystem meldet fehlendes Schreibrecht'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_remove_tree löscht verschachtelte Verzeichnisse'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_group_crypto belegt das Tresor-Modell'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_group_database überspringt ohne Zugangsdaten'] = static function (HcAssert $t): void {
         ...
         */
        $faelle['hc_group_database prüft Version, Paketgröße und JSON-Spalten'] = static function (HcAssert $t): void {
            $host = (string) getenv('HC_TEST_DB_HOST');
            if ($host === '') {
                $t->skip('kein Server angegeben (HC_TEST_DB_HOST), nichts zu prüfen');
            }
        ...
        $faelle['hc_run_longrun wartet lokal und meldet die Dauer'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_run_longrun begrenzt die Wartezeit'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_run_longrun verlangt für den Ferntest eine eigene URL'] = static function (HcAssert $t): void {
        ...
        $faelle['hc_run_smtp bricht ohne Host ab und verrät kein Passwort'] = static function (HcAssert $t): void {
        ...
        return array_map(static fn(\Closure $f): array => [$f], $faelle);
    }
}
